    ...
    return new Promise((resolve, reject) => {
      ...
      bcrypt.hash(adminData.password, 10, (err, hashedPassword) => {
        ...
        if (err) {
          ...
          const qs = "INSERT INTO admin (email, password, name ) VALUES (?, ?, ?)";
          db.query(qs, [adminData.email, hashedPassword, adminData.name], (err, result) => {
            if (err) {
              reject(err);
            } else {
              // If admin is successfully registered, resolve with admin data
              resolve({ id: result.insertId, email: adminData.email });
            }
          });
        }
      });
