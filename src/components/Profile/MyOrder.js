...
export default function MyOrder() {
  ...
  const filteredOrders = dataGetOrderCust?.filter(order => {
            ...
            <Table striped border hover>
              <thead style={{ backgroundColor: "#F7F7F7", border: "none" }} className={styles.headertable}>
                <tr>
                  ...
                  <th style={{ width: "15%" }}>Invoice <img src={sort} alt="Sort" /></th>
                  <th style={{ width: "30%" }}>Product <img src={sort} alt="Sort" /></th>
                  <th style={{ width: "20%" }}>Payment <img src={sort} alt="Sort" /></th>
                  <th style={{ width: "10%", textAlign: "center" }}>Total <img src={sort} alt="Sort" /></th>
                  <th style={{ width: "20%", textAlign: "center" }}>Dates <img src={sort} alt="Sort" /></th>
                </tr>
              </thead>
              <tbody>
                {filteredOrders.map((order, index) => (
                  <tr key={index}>
                    ...
                    <td>{formatRupiah(Number(order.amount))}</td>
